# 🔎 Jour 03 — Recherche

[← Tableau de bord](./README.md) · [Exercices guidés →](./02-guides.md)

> [!NOTE]
> Le but n'est pas de copier une définition. Prépare une explication simple et un exemple personnel.

## Mode d'emploi

1. Recherche la notion.
2. Reformule la réponse avec tes propres mots.
3. Ajoute un petit exemple lorsque c'est possible.
4. Coche la question une fois que tu peux l'expliquer sans lire.

## Questions


### Question 01

> Quelle est la différence entre un *paramètre* et un *argument* d'une fonction ?

- [ ] J'ai recherché et compris la notion.

**Ma réponse :**
Le paramètre est la variable écrite dans la déclaration de la fonction.
L’argument est la valeur qu'on donne à la fonction lorsqu'on l'appelle.
> À compléter avec mes propres mots.

---

### Question 02

> Quelle est la différence entre l'instruction `return` et `console.log()` dans une fonction ?

- [ ] J'ai recherché et compris la notion.

**Ma réponse :**

return : renvoie une valeur à l'extérieur de la fonction.
console.log() :affiche une valeur dans la console, mais ne la renvoie pas.
> À compléter avec mes propres mots.

---

### Question 03

> Que se passe-t-il si l'on oublie le mot-clé `return` dans une fonction censée renvoyer une valeur ?

- [ ] J'ai recherché et compris la notion.

**Ma réponse :**
 la fonction renvoie automatiquement undefined
> À compléter avec mes propres mots.

---

### Question 04

> Qu'est-ce que la **portée (scope)** des variables en JavaScript ? Une variable définie dans une fonction avec `let` est-elle accessible à l'extérieur ?

- [ ] J'ai recherché et compris la notion.

**Ma réponse :**
La portée (scope) désigne l'endroit du code où une variable est accessible:
 une variable déclarée avec let à l'intérieur d'une fonction n'est pas accessible à l'extérieur de cette fonction: variable locale
> À compléter avec mes propres mots.

---

### Question 05

> Quelle est la syntaxe d'une **fonction fléchée** (arrow function) par rapport à une fonction classique ?

- [ ] J'ai recherché et compris la notion.

**Ma réponse :**
1: 
function nomFunction(parametre) {
    // instructions
}

2: 
const nomFunction = (parametre) => {
    return ...
}
> À compléter avec mes propres mots.

---

### Question 06

> Dans quel cas peut-on omettre les accolades `{}` et le mot-clé `return` dans une fonction fléchée ?

- [ ] J'ai recherché et compris la notion.

**Ma réponse :**
lorsqu'il y a une seule expression à retourner.

> À compléter avec mes propres mots.

---

### Question 07

> Qu'est-ce que le *Hoisting* (hissage) en JavaScript, et comment s'applique-t-il aux fonctions classiques `function nom()` versus les variables `const maFunc = () => {}` ?

- [ ] J'ai recherché et compris la notion.

**Ma réponse :**
Le hoisting traite la déclaration classique et l'expression de fonction de manière radicalement différente :La déclaration de fonction est entièrement hissée. On peut donc l'appeler avant sa déclaration 

> À compléter avec mes propres mots.

---

### Question 08

> Qu'est-ce qu'une fonction **Callback** ?

Une callback est une fonction passée en argument à une autre fonction afin d'être exécutée plus tard, après un traitement ou lorsqu'un événement particulier se produit.
- [ ] J'ai recherché et compris la notion.

**Ma réponse :**

> À compléter avec mes propres mots.

---

### Question 09

> Peut-on donner une valeur par défaut à un paramètre (ex: `function saluer(nom = "Invité")`) ?

- [ ] J'ai recherché et compris la notion.

**Ma réponse :
oui 
expl: 
function saluer(nom = "Invité") {
    console.log("Bonjour " + nom);
}
affiche bonjour invité
> À compléter avec mes propres mots.

---

### Question 10

> Qu'est-ce que la *récursivité* en programmation ?

- [ ] J'ai recherché et compris la notion.

**Ma réponse :**
La récursivité signifie qu’une fonction s’appelle elle-même.

> À compléter avec mes propres mots.


## ✅ Validation de la recherche

- [ ] Je peux expliquer au moins trois réponses sans lire mes notes.
- [ ] J'ai noté les notions que je dois encore clarifier.
